...
export function AIAnalyticsPage() {
  ...
  async function runMl() {
    ...
    setMessage(null);
    try {
      const { data, error } = await supabase.rpc('run_advanced_report_ml_v3', {
        p_organization_id: organizationId,
        p_period_start: periodStart,
        p_period_end: periodEnd,
        p_triggered_by: 'frontend:AIAnalyticsPage'
      });
      if (error) throw error;
      const row = Array.isArray(data) && data.length ? data[0] : {};
      setMessage(`ML v3 finished. Raw rows scanned: ${row.raw_rows_scanned ?? 0}. Features: ${row.generated_feature_rows ?? 0}. Insights: ${row.generated_insight_rows ?? 0}.`);
      await load();
    } catch (error) {
    ...
    <div className="page-stack">
      <section className="hero-card ai-hero">
        <div>
          ...
          <p>The SQL seed imports raw report rows only. The ML function generates analytics and stamps generated rows with lineage fields.</p>
        </div>
